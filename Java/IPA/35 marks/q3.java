...
public class q3 {
    public static void main(String[] args) {
        ...
        if(result>0) {
            ...
            System.out.println("There are no cleared students in this location");
        }

        Institution ins = updateInstitutionGrade(arr, name);
        if(ins == null) {
            System.out.println("No institution is available with the specified name");
        }
        else {
            System.out.println(ins.name + "::" + ins.grade);
        }
    }
    public static int findNumClearancedByLoc(Institution[] arr, String location) {
        int sum = 0;
        for(int i=0;i<arr.length;i++) {
            if(arr[i].location.equals(location)) {
...
class Institution {
    ...
    public Institution(int id, String name, int noOfStudentsPlaced, int noOfStudentsCleared,String location) {
        this.id = id;
        this.name = name;
